    ...
    rollupOptions: {
      output: {
        manualChunks(id) {
          ...
          if (id.includes('react/') || id.includes('react-dom/') || id.includes('react-router-dom/') || id.includes('react-router/')) return 'vendor';
          if (id.includes('framer-motion/')) return 'motion';
          if (id.includes('@simplewebauthn/') || id.includes('qrcode/')) return 'auth-libs';
          // Let Vite's default chunking handle other node_modules dependencies
          // to prevent them from being bundled into the main 'vendor' chunk.
          return;
        }
      }
    }
