...
export default function Settings() {
  ...
  useEffect(() => {
    ...
    patchPreferences({ themePref: preference });
  }, [preference]);
              ...
              <div style={{ display: 'flex', gap: 6 }}>
                {[
                  ...
                  <button
                    key={id}
                    onClick={() => {
                      setColorBlindMode(id);
                      patchPreferences({ colorBlindMode: id });
                    }}
                    style={{
                      padding: '5px 10px', borderRadius: 8, border: `1px solid ${colorBlindMode === id ? t.accentBorder : t.border}`,
                      fontFamily: F, fontSize: 12, cursor: 'pointer',
                      background: colorBlindMode === id ? t.accentDim : 'transparent',
                      color: colorBlindMode === id ? t.accent : t.text2,
                      transition: 'all 0.15s',
                    }}
                  >
                    {label}
